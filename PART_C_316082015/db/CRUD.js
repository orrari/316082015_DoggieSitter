...
const getMyTrips = (req, res) => {
    var query = "SELECT t.address, t.description, t.dog_name, t.end_time, t.img_path, t.price, t.start_time, DATE_FORMAT(t.trip_date, '%d/%m/%Y') as trip_date, u.name as owner_name, u.phone_number as owner_phone FROM trips t INNER JOIN trips_requests r ON t.trip_id = r.trip_id INNER JOIN users u ON t.owner_email = u.email WHERE r.requested_by like ? AND request_status = 'approved';"
    sql.query(query, req.cookies.userEmail + "%",  (err, mysqlres) => {
...
const getMyRequests = (req, res) => {
    ...
    // TODO: change query 
    var query = "SELECT t.end_time, t.start_time, DATE_FORMAT(t.trip_date, '%d/%m/%Y') as trip_date, u.name, u.age, u.phone_number, u.about_me FROM trips t INNER JOIN trips_requests r ON t.trip_id = r.trip_id AND t.trip_status = r.request_status INNER JOIN users u ON r.requested_by = u.email WHERE t.owner_email like ? AND request_status = 'pending';"
    ...
    sql.query(query, req.cookies.userEmail + "%", (err, mysqlres) => {
        ...
        if (err) {
            console.log("error: ", err);
            res.status(400).send({message: "error in getting my requests: " + err});
            return;
        }
...
const createNewRequest = (req, res)=>{
    ...
    sql.query("INSERT INTO trips_requests SET ?", ReqData, (err, mysqlres) => {
        if (err) {
            console.log("error: ", err);
            res.status(400).send({message: "error in create user: " + err + ReqData});
            return;
        }
...
module.exports = {checkLoginInputs, SignUpCheck, getMyTrips, getAvailableTrips, getMyRequests, removeTrip, createNewRequest};